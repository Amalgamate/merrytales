...
export function ChangePassword() {
  ...
  return <AuthShell eyebrow="Security check" title="Set your own password" description="Your temporary password can only be used once. Set a unique 16-character password to unlock the workspace.">
    <form onSubmit={submit} className="space-y-5">
      ...
      <div className="rounded-xl border border-primary/15 bg-pink-50/60 p-4 text-sm text-[#3a2340]"><ShieldCheck className="mb-2 h-5 w-5 text-primary" />This step is required before you can access admin controls.</div>
      <label className="block text-sm font-bold text-[#25253f]">Temporary password<div className="relative mt-2"><KeyRound className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" /><Input required autoComplete="current-password" type="password" value={currentPassword} onChange={event => setCurrentPassword(event.target.value)} className="h-13 rounded-xl bg-white pl-11 text-base" /></div></label>
