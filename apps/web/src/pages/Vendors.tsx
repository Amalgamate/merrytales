...
import { Calendar, Filter, Loader2, MapPin, Search } from 'lucide-react';
...
export function Vendors() {
  ...
  useEffect(() => {
    setLoading(true);
    setError('');
    fetchVendors({ q: searchQuery || undefined, city: location || undefined, limit: 48 })
      .then(({ items }) => setVendors(items.map(toVendorCard)))
  ...
  return (
    <div className="pt-20 min-h-screen bg-background pb-24">
      <PageHero
        ...
        subtitle="Discover and book verified event professionals across Nairobi and Kenya."
      ...
      >
        <div className="bg-white rounded-2xl md:rounded-full p-2 grid grid-cols-2 md:grid-cols-[1.4fr_1fr_1fr_auto] shadow-xl max-w-4xl mx-auto border border-border-soft">
          <div className="relative col-span-2 md:col-span-1 flex items-center px-4 h-12 border-b md:border-b-0 md:border-r border-gray-200">
            ...
            <input type="text" placeholder="What are you looking for?" className="w-full focus:outline-none text-foreground bg-transparent" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} autoComplete="off" />
      ...
      <div className="max-w-7xl mx-auto px-4 mt-32 sm:mt-28 md:mt-16 mb-12">
        <div className="flex overflow-x-auto pb-4 hide-scrollbar space-x-3">
          {marketplaceCategories.slice(0, 8).map(({ name: cat }) => (
            <button key={cat} onClick={() => setSearchQuery(cat)} className="flex-shrink-0 px-6 py-3 rounded-full border border-border-soft bg-white hover:border-primary hover:text-primary transition-colors font-medium shadow-sm">{cat}</button>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-2xl font-bold">{searchQuery || location ? `${filteredVendors.length} matching vendors` : 'Verified vendors'}</h2>
        ...
        {loading ? <div className="grid min-h-64 place-items-center text-gray-400"><Loader2 className="h-8 w-8 animate-spin" /></div> : filteredVendors.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{filteredVendors.map((vendor) => <VendorCard key={vendor.id} {...vendor} />)}</div>
        ) : (
          <div className="text-center py-20 bg-white rounded-3xl border border-border-soft">
            ...
            <Button className="mt-4 rounded-full" onClick={() => { setSearchQuery(''); setLocation(''); }}>Clear Search</Button>
          </div>
        )}
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 mt-24 mb-12">
        <div className="bg-lavender-light rounded-3xl p-8 md:p-16 text-center border border-lavender-soft">
          <h2 className="text-3xl font-bold mb-4">Are you an event vendor?</h2>
          <p className="text-gray-700 mb-8 max-w-2xl mx-auto">Join the Merry Tales marketplace to reach more couples, manage bookings, and get paid through verified checkout.</p>
          <Link to="/vendor/join"><Button className="rounded-full px-8 py-6 font-bold shadow-soft">Join Merry Tales Marketplace</Button></Link>
        </div>
      </div>
    </div>
  );
