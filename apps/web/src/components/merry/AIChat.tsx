...
import { Bot, CalendarDays as Sparkles, Loader2, MessageCircle, Send, X } from 'lucide-react';
...
export function AIChat() {
  ...
  return <div className="fixed bottom-20 right-4 z-[80] sm:bottom-6 sm:right-6">
    ...
    <button onClick={() => setOpen((value) => !value)} aria-label={open ? 'Close Merry assistant' : 'Open Merry assistant'} className="ml-auto flex h-14 items-center gap-2 rounded-full bg-primary px-5 font-extrabold text-white shadow-[0_12px_30px_rgba(233,75,135,.38)] transition hover:-translate-y-1 hover:bg-[#d92f73]">{open ? <X className="h-5 w-5" /> : <><MessageCircle className="h-5 w-5" /><span>Ask Merry</span><Bot className="h-4 w-4 opacity-70" /></>}</button>
  </div>;
}
