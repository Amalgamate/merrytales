...
export function Navbar() {
  ...
  return (
    <nav className="fixed w-full z-50 bg-background/95 backdrop-blur-md border-b border-border-soft">
      ...
      {panel === 'location' && locationLabel !== 'Near me' && <div className="absolute right-4 top-[72px] z-[70] w-[min(350px,calc(100vw-32px))] rounded-2xl border bg-white p-5 text-[#171735] shadow-2xl sm:right-6"><div className="flex items-start justify-between"><div><h3 className="font-extrabold">Find events near you</h3><p className="mt-1 text-xs leading-5 text-slate-500">Turn on approximate location to surface nearby vendors and services. You can continue without it.</p></div><button onClick={closeLocationPrompt} aria-label="Close location prompt"><X className="h-4 w-4" /></button></div><button onClick={detectLocation} disabled={locating} className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary font-bold text-white disabled:opacity-60">{locating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Navigation className="h-4 w-4" />}{locating ? 'Detecting…' : 'Turn on location'}</button>{locationLabel !== 'Choose location' && <p className="mt-3 flex items-center gap-2 text-xs font-semibold text-slate-500"><MapPin className="h-3.5 w-3.5 text-primary" />{locationLabel}</p>}</div>}
